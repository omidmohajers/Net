...
namespace PA.Net.Clients
{
    ...
    public class LocalTcpClient : TcpNetClient
    {
        ...
        {
            Package pak = new Package(CommandType.SendClientList, ServerIP, IP, "", null);
            pak.UserID = UserID;
            pak.SenderIP = IP;
            Send(Package.ToByteArray(pak));
        }
        ...
        public void SendFileInfo(string fileName, long room)
        {
            if (!File.Exists(fileName))
                throw new FileNotFoundException();
            FileInfo info = new FileInfo(fileName);
            string data = string.Format("{0}:{1}", info.Name, info.Length);
            Package pak = new Package(CommandType.FileOrder, IPAddress.Broadcast, IP, data, room);
            pak.UserID = UserID;
            Send(Package.ToByteArray(pak));
        }
        public void StartBroadcastingVideo(long room)
        {
            Package pak = new Package(CommandType.StartBroadcastingVideo, IPAddress.Broadcast, IP, IP.ToString(), room);
            pak.UserID = UserID;
            pak.SenderIP = IP;
            Send(Package.ToByteArray(pak));
        }
